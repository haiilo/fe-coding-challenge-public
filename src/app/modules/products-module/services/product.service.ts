...
export class ProductService {
  ...
  private initialProduct: Product | undefined = this.loadSelectedProduct();
  private selectedProductSource = new BehaviorSubject<Product | undefined>(
    this.initialProduct
  );
  ...
  get(page: number): Observable<Page<Product>> {
    ...
    if (page > ProductService.PAGE_COUNT) {
      ...
      const content = this.getContent(
        randNumber({ min: 1, max: ProductService.PAGE_SIZE })
      );
      res = { more: false, content };
    } else if (page >= 0) {
  ...
  private loadSelectedProduct(): Product | undefined {
